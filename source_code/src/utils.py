...
from sampling import cifar_iid, mnist_noniid, cifar_noniid, mnist_iid, mnist_noniid, svhn_iid, svhn_noniid, iid, cifar_non_iid_test
...
def adj_matrix_converter_flatten(model_list):
    ...
    # adjacency_matrix_map = np.ones((num, num))
    # for i in range(num):
    #     for j in range(num):
    #         if adjacency_matrix[i][j] >= (sum(adjacency_matrix[i])-1)/(num-1) or i==j:
    #             adjacency_matrix_map[i][j] = 1
    #         elif adjacency_matrix[i][j] < (sum(adjacency_matrix[i])-1)/(num-1) and i != j:
    #             adjacency_matrix_map[i][j] = 0
...
def get_datasets(args):
    ...
    if args.dataset == 'cifar':
        ...
        if args.iid:
            ...
            dict_users_train, dict_users_test = cifar_non_iid_test(train_dataset, args.num_users, args)
    ...
    if args.dataset == 'svhn':
        ...
        if args.iid:
            ...
            dict_users_train, dict_users_test = svhn_noniid(train_dataset, args.num_users, args)
