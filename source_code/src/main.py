...
from models import MLP, CNNMnist, CNNFashion_Mnist, CNNCifar, CNNSVHN, CNNCifar_large
...
if __name__ == '__main__':
    ...
    if args.model == 'cnn':
        ...
        #     global_model = CNNFashion_Mnist(args=args).to(device)
            ...
            global_model = CNNCifar_large(args=args).to(device)
    ...
    save_path = '../12_29_main_exp/fed_{}_{}_{}_iid[{}]_E[{}]_mp_{}_cus_test_{}_cluster_{}_frac_{}_user_num_{}/'.format(args.dataset, args.model, args.epochs,
  ...
  # Create a new directory because it does not exist 
    ...
    with open(save_path + 'local_avg_test_accuracy_list.txt', 'w') as filehandle:
        for listitem in local_avg_test_accuracy_list:
            filehandle.write('%s\n' % listitem) 
    print("*******last 10 avg local acc****************")
    print("{}".format(sum(local_avg_test_accuracy_list[-10:])/10))
    print("********************************************")
